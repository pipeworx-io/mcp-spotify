...
`tools/list` at `https://gateway.pipeworx.io/spotify/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/spotify_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"The Weeknd"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/spotify_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
